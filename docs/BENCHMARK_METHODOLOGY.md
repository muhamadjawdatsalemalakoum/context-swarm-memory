...
**Reproducibility:** see [REPRODUCING.md](REPRODUCING.md). The cache-first design means `npm run bench:replay <runId>` regenerates the headline numbers in under 5 minutes with zero LLM calls. The synthetic PaySwift corpus + decisions ledger + queries are versioned in-repo under `data/eval/corpus-synthetic/`. BABILong raw files are not redistributed (subject to the original dataset license); the fetch script pulls them from HF and caches locally.
