...
("Brief T1"). Grounding HEAD at dispatch: `2e086fd` (branch cut from
`5cbe311`).
...
**Deletable post-merge (line numbers at `5cbe311`; ≈640 of 1,224 lines):**
...
1. `src/core/coverage.ts`, `src/core/types.ts`, `src/core/schemas.ts`,
   ...
   `cfcb8af` + the helper from `71d2a3b`).
2. `src/core/ask.ts` — orchestrator applies the three-part diff from
   `71d2a3b` (imports + intent/budget resolution; three
...
3. `src/eval/baselines/csm.ts` — orchestrator applies the three-part diff
   from `71d2a3b` (timeline tier in `baseRetrievalOrder`; TIMELINE block in
...
consumed the chronicle (commit e973617): then event_ordering cov@24 rose
