...
<p align="center"><img src="docs/assets/scaling.svg" width="560" alt="Accuracy as memory scales 10x: CSM holds (93->90%), vanilla RAG degrades (97->83%), long-context stays collapsed (37->30%)"></p>

At 1M tokens, CSM stays ahead of vanilla RAG (27/30 vs 25/30 — within the noise band; see calibration below) and beats long-context **27–9 (18–0 discordant pairs, exact McNemar p<0.0001)** — at **zero LLM-indexing cost** (keyword/tag routing plus a local embedding recall floor, no LLM-generated index), where long-context physically can't fit the corpus (8K = 0.06% of 1M) and embedding-RAG degrades under the added distractors. *The more CSM remembers, the more its edge shows.*

> Honest calibration: single-trial; CSM's absolute accuracy is ~27–30/30 across runs (Gemma at temp=0 is not bitwise-deterministic across processes; the current-pipeline rows are 28/30 at 100K and 27/30 at 1M), so the robust claim is **"does not degrade as memory grows, while the alternatives do"** — not that its raw score climbs.

## Results — current evidence status

Same 30-query benchmark, same 100K-token corpus, same local Gemma 4 31B answering for **every** system (8K context, temp 0) — only the retrieval/memory layer differs. CSM beats **LightRAG**, a 2025 graph-RAG comparator, on accuracy in every run pair and leads clearly on **citation grounding quality**:

<p align="center"><img src="docs/assets/citation-f1.svg" width="560" alt="Citation F1 by system at 100K: CSM 0.470, hybrid RAG 0.455, vanilla RAG 0.446, LightRAG 0.265, long-context 0.067"></p>

- **vs LightRAG (2025 graph-RAG): CSM leads** — citation F1 0.470 vs 0.265, accuracy 28/30 vs 24/30 on the current-pipeline rows (discordant 6–2, exact McNemar p=0.289; an earlier 30/30 run reached p=0.031 — the lead is consistent across runs, single-run significance is not, and we say so). Mem0 and HippoRAG could not be made to run locally on consumer hardware — documented as **blocked, not beaten** ([`SOTA_COMPARISON.md`](SOTA_COMPARISON.md)).
